...
class Manager():
    def __init__(self, env, env_name, save_dir):
        self.env = env
        ...
        self.done = False
        self.trunc = False
        self.done_or_trunc = False
        self.iter_time = 0
        self.obs, self.info = self.env.reset()
        self.info['state'] = [self.info['state']]  # For RHCWrapper
        self.language_instruction = env.get_task()["language_instruction"]
        self.image = self.obs["image_primary"][-1]

    def step(self, action, save=False):
        """ Step env with episode cache

        Args:
            action (np.ndarray): Action should be shape [4]
        """
        # print(self.obs["image_primary"].shape)
        # print(self.info["state"].shape)
        self.images.append(self.obs["image_primary"][-1])
        
        if save:
            self.episode.append({
                'image': np.asarray(self.obs["image_primary"][-1], dtype=np.uint8),  # Image uint8
                'wrist_image': np.asarray(np.random.rand(256, 256, 3) * 255, dtype=np.uint8),  # Placeholder wrist image
                'state': np.asarray(self.info["state"][-1][:4], dtype=np.float32),  # State from the environment
                'action': np.asarray(action, dtype=np.float32),  # Action taken
                # 'reward': np.asarray(reward, dtype=np.float32),  # Reward received
                'language_instruction': self.language_instruction[0],  # Dummy language instruction
            })

        obs, reward, done, trunc, info = self.env.step(np.array(action)) 
    ...
    def __del__(self):
        if self.save_dir:
            np.savez_compressed(
                os.path.join(
                    self.save_dir, 
                    f"episode_{datetime.datetime.now().strftime('%Y-%m-%d-%H-%M-%S')}-{self.env_name}-{bool(self.done_or_trunc)}.npz".replace("-", "_")), 
